...
def build_history_records(long_frame: pd.DataFrame) -> tuple[list[Employee], list[PayrollHistory]]:
    ...
    for idx, row in long_frame.iterrows():
        ...
        status_value = normalize_status_string(row.get("raw_value"))
        if status_value in {"TS", "OM", "KL", "ST", "NV"}:
            long_frame.at[idx, "status"] = status_value
            long_frame.at[idx, "salary_value"] = 0.0
            continue

        numeric_value = parse_salary_value(row.get("raw_value"))
    ...
    prior_has_value = long_frame.groupby("emp_id")["has_value"].shift(1).fillna(False)
    prior_status = long_frame.groupby("emp_id")["status"].shift(1)
    current_blank = ~long_frame["has_value"]
    nv_mask = current_blank & prior_has_value & ~prior_status.eq("NV")
    long_frame.loc[nv_mask, "status"] = "NV"
    long_frame.loc[nv_mask, "resign_date"] = long_frame.loc[nv_mask, "month"].dt.date

    drop_mask = current_blank & (~prior_has_value | prior_status.eq("NV"))
    ...
    long_frame["status_in_month"] = long_frame["status_in_month"].replace({"None": "Active"})
